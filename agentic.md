...
To access all agents we'll need to play this use case, you have to access "**Ai Assistant**", this is possible either through AJO or from the AEP Sandbox home page. The "Ai Assistant" is your interface UI, your prompt, where all interaction with Agent Orchestrator starts. Then Agent Orchestrator will decide which agents to call based on your text prompt requests.
...
- Let's start building our journey with the creation of an audience, for that, let's use Ai Assistant prompt and use the following text/request to AEP Agent Orchestrator: "_I want to create an audience with profiles which propensity score is higher than 50_" (you can obviouly use your own text and language if you prefer)
